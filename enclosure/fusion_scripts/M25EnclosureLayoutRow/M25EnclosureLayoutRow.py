"""
M25 Remote Enclosure - VARIANT B, single button row
===================================================
135 x 85 x 70 mm. Four 19 mm illuminated buttons in one row across the top,
joystick centred below, indicator LEDs in a column on the left, and the right
pocket kept clear as a display reservation.
...
Companion script: m25_enclosure_grid.py (variant A, 2x2 cluster, 125 x 85 x 70).
...
# ---------------- shell ----------------
OUT_W, OUT_H, TOTAL_H = 135.0, 85.0, 70.0
...
BTN_ROW_Y, BTN_PITCH = 22.5, 30.0
LED_HOLE = 5.0
JOY_ENV, JOY_HOLE, JOY_DEPTH = 40.0, 30.0, 30.0
JOY = (0.0, -15.0)

# left to right. E-stop at the far end so it is findable by feel;
# Power at the opposite end so the two "do not hit by accident" controls
# cannot be confused.
BUTTONS = [('Power lila', -45.0, BTN_ROW_Y),
           ('Hill-hold gruen', -15.0, BTN_ROW_Y),
           ('Assist gelb', 15.0, BTN_ROW_Y),
           ('E-stop rot', 45.0, BTN_ROW_Y)]

# all indicators in one left column, keeping the right pocket clear
LEDS = [('BLE weiss', -47.5, -4.0), ('Status rot', -47.5, -16.0)]
RESERVED_LED = ('Battery GPIO27', -47.5, -28.0)   # reserved in device_config.h

# future display, PCB outlines only - drawn as construction geometry
DISPLAYS = [('0.96in SSD1306', 27.0, 27.0), ('1.3in SH1106', 35.0, 33.0)]
DISP_C = (42.0, -16.0)
...
def run(_context: str):
    ...
    circle(sk, RESERVED_LED[1], RESERVED_LED[2], LED_HOLE, True)
    circle(sk, JOY[0], JOY[1], JOY_HOLE)
    rect(sk, JOY[0] - JOY_ENV / 2, JOY[1] - JOY_ENV / 2,
         JOY[0] + JOY_ENV / 2, JOY[1] + JOY_ENV / 2, True)
    for _, w, h in DISPLAYS:
        rect(sk, DISP_C[0] - w / 2, DISP_C[1] - h / 2,
             DISP_C[0] + w / 2, DISP_C[1] + h / 2, True)
    sk.isComputeDeferred = False
...
def _audit(top_z):
    ...
    row_bot = BTN_ROW_Y - BTN_ENV / 2
    joy_l, joy_r = JOY[0] - JOY_ENV / 2, JOY[0] + JOY_ENV / 2

    print('=== VARIANT B, single row - %.0f x %.0f x %.0f mm ===' % (OUT_W, OUT_H, TOTAL_H))
    ...
    print('-- button row --')
    print('span %.1f mm, pitch %.1f, bezel gap %.1f' %
          (BTN_PITCH * (len(BUTTONS) - 1) + BTN_ENV, BTN_PITCH, BTN_PITCH - BTN_ENV))
    print('outer bezel to side wall: %.1f mm' %
          (inner_x - max(abs(x) for _, x, _ in BUTTONS) - BTN_ENV / 2))
    print('row bezel to top wall:    %.1f mm' % (inner_y - BTN_ROW_Y - BTN_ENV / 2))
    print('row bezel to joystick:    %.1f mm' % (row_bot - (JOY[1] + JOY_ENV / 2)))
    print('joystick to bottom wall:  %.1f mm' % (JOY[1] - JOY_ENV / 2 + inner_y))

    print('-- indicator column --')
    pitch = abs(LEDS[0][2] - LEDS[1][2])
    print('x = %.1f, pitch %.1f, body gap %.1f mm' % (LEDS[0][1], pitch, pitch - LED_HOLE))
    print('column to joystick cap:   %.1f mm' % (joy_l - (LEDS[0][1] + LED_HOLE / 2)))
    print('lowest LED to bottom wall: %.1f mm' % (RESERVED_LED[2] - LED_HOLE / 2 + inner_y))

    print('-- display reservation, right pocket --')
    print('pocket: x %.1f..%.1f (%.1f wide), y %.1f..%.1f (%.1f tall)' %
          (joy_r, inner_x, inner_x - joy_r, -inner_y, row_bot, row_bot + inner_y))
    print('band z %.1f .. %.1f, board top at %.1f' % (top_z - 10.0, top_z, tall))
    for n, w, h in DISPLAYS:
        print('%-16s %2.0f x %2.0f -> joystick %.1f, wall %.1f, top %.1f, bottom %.1f'
              % (n, w, h, (DISP_C[0] - w / 2) - joy_r,
                 inner_x - (DISP_C[0] + w / 2),
                 row_bot - (DISP_C[1] + h / 2),
                 (DISP_C[1] - h / 2) + inner_y))
